  ...
  win.document.write(`<!DOCTYPE html><html><head><title>Etiquetas QR — Control de Engrase</title>
    ...
    <div class="grid">
      ${items.map(({ e, dataUrl }) => `
        <div class="label">
          <img src="${dataUrl}"/>
          <h4>${esc(e.code)}${e.shortCode ? ' · ' + e.shortCode : ''}</h4>
...
function openEquipmentQR(equipment) {
  ...
  $('#btn-print-qr').addEventListener('click', () => {
    ...
    win.document.write(`<html><body style="text-align:center; font-family:sans-serif; padding:30px">
      <h2>${esc(equipment.code)}${equipment.shortCode ? ' · ' + equipment.shortCode : ''}</h2>
...
/* ---------- Plan de engrase imprimible (para entregar en físico al personal) ----------
   Arma una hoja por día con los equipos que toca engrasar, separados por turno,
   con casillas en blanco para que el lubricador marque a mano en campo. */
async function printGreasePlan(rango, equipos, plans, types) {
  ...
  if (rango === 'semana') {
    ...
    inicio.setDate(inicio.getDate() - inicio.getDay() + 1); // arranca el lunes
    ...
    for (let d = new Date(inicio); d <= fin; d.setDate(d.getDate() + 1)) {
      if (d.getDay() !== 0) dias.push(new Date(d)); // salta domingos
    }
  }

  const nombreLoc = id => (locations.find(l => l.id === id) || {}).name || '';
  const nombreTipo = id => (types.find(t => t.id === id) || {}).name || '';

  function equiposDelDia(fecha) {
    const nombreDia = WEEKDAY_NAMES[fecha.getDay()];
    const resultado = [];
    for (const eq of equipos) {
      if (eq.status !== 'Operativo') continue;
      const plan = plans.find(p => p.equipmentId === eq.id);
      if (!plan) continue;

      if (plan.controlType === 'Día y turno de la semana') {
        if ((plan.assignedDays || []).includes(nombreDia)) resultado.push({ eq, motivo: 'Programado' });
      } else {
        // Por horas: solo aparece en el día de HOY si ya está vencido o próximo
        if (fecha.getTime() !== hoy.getTime()) continue;
        const proximo = plan.lastGreaseHour + plan.frequency;
        const restante = proximo - eq.hourmeter;
        if (restante <= (plan.alertYellowHours || 0)) {
          resultado.push({ eq, motivo: restante < 0 ? `VENCIDO (${fmt(Math.abs(restante))} h)` : `Faltan ${fmt(restante)} h` });
        }
      }
    }
    return resultado.sort((a, b) => (a.eq.shiftId || '').localeCompare(b.eq.shiftId || '') || a.eq.code.localeCompare(b.eq.code));
  }

  const bloques = dias.map(d => {
    const lista = equiposDelDia(d);
    const fechaTxt = `${WEEKDAY_NAMES[d.getDay()]} ${d.getDate()}/${d.getMonth() + 1}/${d.getFullYear()}`;
    if (!lista.length) {
      return `<div class="dia"><h3>${fechaTxt}</h3><p class="vacio">Sin equipos programados para este día.</p></div>`;
    }
    const porTurno = { shift_dia: [], shift_noche: [] };
    lista.forEach(x => { (porTurno[x.eq.shiftId] || porTurno.shift_dia).push(x); });

    const tablaTurno = (titulo, items) => !items.length ? '' : `
      <h4>${titulo}</h4>
      <table>
        <thead><tr>
          <th style="width:28px">✓</th><th>Código</th><th>Equipo</th><th>Ubicación</th>
          <th>Categoría</th><th>Motivo</th><th style="width:90px">Horóm. real</th><th style="width:110px">Firma</th>
        </tr></thead>
        <tbody>
          ${items.map(({ eq, motivo }) => `<tr>
            <td class="check"></td>
            <td class="mono"><b>${esc(eq.code)}</b>${eq.shortCode ? ` · ${esc(eq.shortCode)}` : ''}</td>
            <td>${esc(eq.brand)} ${esc(eq.model)}</td>
            <td>${esc(nombreLoc(eq.locationId))}</td>
            <td>${esc(nombreTipo(eq.typeId))}</td>
            <td>${esc(motivo)}</td>
            <td></td><td></td>
          </tr>`).join('')}
        </tbody>
    ...
    return `<div class="dia">
      <h3>${fechaTxt} <span class="conteo">(${lista.length} equipo${lista.length !== 1 ? 's' : ''})</span></h3>
      ${tablaTurno('Turno Día', porTurno.shift_dia)}
      ${tablaTurno('Turno Noche', porTurno.shift_noche)}
    </div>`;
  }).join('');
  ...
  win.document.write(`<!DOCTYPE html><html><head>
    <title>Plan de engrase — ${rango === 'semana' ? 'Semana' : 'Mes'}</title>
    <style>
      ...
      .dia { page-break-inside: avoid; margin-bottom: 18px; }
      h3 { font-size: 14px; background: #eee; padding: 6px 8px; margin: 0 0 6px; border-left: 4px solid #E8A33D; }
      .conteo { font-weight: normal; color: #666; font-size: 11px; }
      h4 { font-size: 12px; margin: 8px 0 4px; color: #444; }
      ...
      .check { border: 1px solid #666; }
      .mono { font-family: 'Courier New', monospace; }
      .vacio { color: #888; font-style: italic; padding: 4px 8px; }
      @media print { body { margin: 8px; } }
    </style></head><body>
    <h1>Plan de Engrase — ${rango === 'semana' ? 'Semana' : `Mes de ${hoy.toLocaleDateString('es', { month: 'long', year: 'numeric' })}`}</h1>
    <div class="sub">
      Generado el ${fmtDate(nowISO())} por ${esc(App.currentUser.name)} ·
      Marca la casilla al terminar cada equipo, anota el horómetro real y firma.
      ${rango === 'semana' ? 'Los equipos con control por horas aparecen solo el día de hoy, según su estado actual.' : ''}
    </div>
    ${bloques}
  </body></html>`);
  win.document.close();
  ...
  loadExisting.then(existing => {
    ...
    openModal(pointId ? 'Editar punto' : 'Nuevo punto de engrase', `
      <form id="point-form" class="form-grid">
        <label>Sistema<input name="system" value="${esc(p.system)}"/></label>
        <label>Componente<input name="component" value="${esc(p.component)}"/></label>
        <label>Punto de engrase<input required name="point" value="${esc(p.point)}"/></label>
        <label>Tipo de grasa
...
async function renderReportes() {
  ...
  $('#exp-excel').addEventListener('click', () => {
    ...
    const wsResumen = XLSX.utils.aoa_to_sheet([
      ...
      [`Generado: ${fmtDate(nowISO())}  ·  Por: ${esc(App.currentUser.name)}`],
    ...
    const wsHist = XLSX.utils.json_to_sheet(records.map(r => {
      const eq = equipos.find(e => e.id === r.equipmentId);
      return {
        'Fecha': fmtDate(r.date), 'Código': eq ? eq.code : '', 'Equipo': eq ? `${esc(eq.brand)} ${esc(eq.model)}` : '',
  ...
  $('#exp-pdf').addEventListener('click', () => {
    ...
    doc.setFontSize(8.5);
    doc.text(`Generado: ${fmtDate(nowISO())}   ·   Por: ${esc(App.currentUser.name)}   ·   Periodo: ${$('#f-from').value || '—'} a ${$('#f-to').value || '—'}`, 40, 62);
    y = 100;
    ...
    doc.autoTable({
      ...
      body: attention.length ? attention.map(({ e, s }) => [
        s.label, e.code, `${esc(e.brand)} ${esc(e.model)}`, fmt(e.hourmeter) + ' h',
    ...
    applyFilters().forEach(r => {
      const eq = equipos.find(e => e.id === r.equipmentId);
      (r.details || []).filter(d => !d.done).forEach(d => {
        pendientes.push([fmtDate(r.date), eq ? eq.code : '—', d.pointName, d.reason || '(sin motivo)', r.userName]);
      });
      if (r.sinHorometro) {
        pendientes.push([fmtDate(r.date), eq ? eq.code : '—', 'Sin lectura de horómetro', r.noHourmeterReason || 'No se pudo leer', r.userName]);
      }
    });
    if (y > 620) { doc.addPage(); y = 40; }
    doc.setFont(undefined, 'bold'); doc.setFontSize(11); doc.setTextColor(20);
    ...
    doc.autoTable({
      startY: y, margin: { left: 40, right: 40 }, styles: { fontSize: 7.5 }, headStyles: { fillColor: [232, 163, 61] },
      head: [['Fecha', 'Código', 'Punto', 'Motivo', 'Reportado por']],
      body: pendientes.length ? pendientes.slice(0, 40) : [['—', '—', '—', 'Sin puntos pendientes en el periodo', '—']]
    });
    y = doc.lastAutoTable.finalY + 20;
